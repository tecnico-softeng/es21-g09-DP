...
public class QuestionsXmlImport {
    ...
    private void importQuestion(Element questionElement, Course course) {
        ...
        switch (type) {
            ...
            case Question.QuestionTypes.COMBINATION_ITEM_QUESTION:
                questionDetailsDto = importCombinationItemQuestion(questionElement);
                break;
    ...
    private QuestionDetailsDto importMultipleChoiceQuestion(Element questionElement) {
        ...
        for (Element optionElement : questionElement.getChild("options").getChildren("option")) {
            ...
            Integer order = (optionElement.getAttributeValue(ORDER).equals("null")) ? null : Integer.valueOf(optionElement.getAttributeValue(ORDER));
        ...
        multipleChoiceQuestionDto.setOrdered(Boolean.parseBoolean(questionElement.getAttributeValue("ordered")));
    ...
    private QuestionDetailsDto importOpenEndedQuestion(Element questionElement) {
        ...
        questionDto.setAnswer(questionElement.getAttributeValue("answer"));

        return questionDto;
    }

    private QuestionDetailsDto importCombinationItemQuestion(Element questionElement) {
        List<CombOptionDto> optionDtos = new ArrayList<>();
        for (Element optionElement : questionElement.getChild("options").getChildren("combOption")) {
            Integer optionSequence = Integer.valueOf(optionElement.getAttributeValue(SEQUENCE));
            String optionContent = optionElement.getAttributeValue(CONTENT);
            ...
            combOptionDto.setLeft(left);
            combOptionDto.setLink(link);

            optionDtos.add(combOptionDto);
        }
        
        CombinationItemQuestionDto CombinationItemQuestionDto = new CombinationItemQuestionDto();
        CombinationItemQuestionDto.setOptions(optionDtos);
        return CombinationItemQuestionDto;
    }
